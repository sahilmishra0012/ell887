...
def download_and_load_model(model_path):
    ...
    if not os.path.exists('/'.join(splits[3:])):
        ...
        for blob in blobs:
            ...
            if filename.rsplit('/')[-1] != '':
                create_dir = filename.split(splits[3])[1]
                print(create_dir)
                os.makedirs(splits[3]+os.path.dirname(create_dir), mode=0o777, exist_ok=True)
                blob.download_to_filename(splits[3]+filename.split(splits[3])[1])  # Download
        print("Model Downloaded")
    else:
        print("Model Already Downloaded")
    try:
        if model is None:
            model = tf.keras.models.load_model('/'.join(splits[3:]))
            return "Model Loaded"
        else:
